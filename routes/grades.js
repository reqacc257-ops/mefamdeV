...
router.post('/', (req, res) => {
  ...
  try {
    subjects.forEach(s => {
      ...
      if (!subj) return;
      const existing = db.prepare('SELECT * FROM quarterly_grades WHERE student_id = ? AND school_year = ? AND subject = ? AND quarter = ?').get(studentId, schoolYear, subj, quarter);
      if (existing) {
        db.prepare(`
          UPDATE quarterly_grades
          SET grade_value = ?, file_data = ?, status = 'pending', reviewed_by = '', reviewed_at = '', rejection_reason = ''
          WHERE id = ?
...
router.get('/mine', (req, res) => {
  ...
  const rows = db.prepare('SELECT * FROM quarterly_grades WHERE student_id = ? AND school_year = ? ORDER BY subject, quarter').all(studentId, schoolYear);
...
router.get('/student/:studentId/grade-card', (req, res) => {
  ...
  const rows = db.prepare('SELECT subject, quarter, grade_value FROM quarterly_grades WHERE student_id = ? AND school_year = ? AND status = ?').all(studentId, schoolYear, 'approved');
  const grouped = {};
  rows.forEach(r => {
...
router.get('/pending', requireRole('director','edu'), (req, res) => {
  const rows = db.prepare('SELECT * FROM quarterly_grades WHERE status = ? ORDER BY submitted_at DESC').all('pending');
...
router.patch('/:id/approve', requireRole('director','edu'), (req, res) => {
  const id = parseInt(req.params.id, 10);
  const reviewer = req.user?.id || req.user?.username || 'staff';
  db.prepare('UPDATE quarterly_grades SET status = ?, reviewed_by = ?, reviewed_at = ? WHERE id = ?').run('approved', reviewer, new Date().toISOString(), id);
  logger.info('Quarter approved', { id, reviewer });
  res.json({ ok: true });
});

// Admin: reject
router.patch('/:id/reject', requireRole('director','edu'), (req, res) => {
  const id = parseInt(req.params.id, 10);
  const reviewer = req.user?.id || req.user?.username || 'staff';
  const reason = String(req.body.reason || req.body.rejection_reason || '').trim();
  db.prepare('UPDATE quarterly_grades SET status = ?, reviewed_by = ?, reviewed_at = ?, rejection_reason = ? WHERE id = ?').run('rejected', reviewer, new Date().toISOString(), reason, id);
  logger.info('Quarter rejected', { id, reviewer, reason });
  res.json({ ok: true });
});

// Admin: edit a grade (manual correction)
router.patch('/:id', requireRole('director','edu'), (req, res) => {
  const id = parseInt(req.params.id, 10);
  ...
  db.prepare('UPDATE quarterly_grades SET grade_value = ?, last_edited_by = ?, last_edited_at = ? WHERE id = ?').run(gradeVal, editor, new Date().toISOString(), id);
