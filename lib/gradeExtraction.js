...
function parseOcrSpaceText(text) {
  ...
  let current = null;
  let termCount = 4;
  const excludedLabels = /^(learning areas?|term|terms|quarter|quarters|final( grade)?|remarks?|general average|school year)$/i;
  const getGradeNumbers = (line) => (line.match(/\b(?:[6-9]\d|100)(?:\.\d+)?\b/g) || []).map(Number);
  const getLabel = (line) => line
  ...
  const finishSubject = () => {
    if (!current) return;
    const values = termCount === 3
      ? [current.numbers[0] ?? null, current.numbers[1] ?? null, current.numbers[2] ?? null, null, current.numbers[3] ?? null]
      : [current.numbers[0] ?? null, current.numbers[1] ?? null, current.numbers[2] ?? null, current.numbers[3] ?? null, current.numbers[4] ?? null];
    subjects.push({
      name: current.name,
      q1: values[0],
      q2: values[1],
  ...
  String(text || '').split(/\r?\n/).forEach(line => {
    ...
    const numbers = getGradeNumbers(line);
    const label = getLabel(line);
    if (/\bterm\s*1\b/i.test(line) && /\bterm\s*2\b/i.test(line) && /\bterm\s*3\b/i.test(line) && !/\bterm\s*4\b/i.test(line)) {
      termCount = 3;
    }
    if (match) {
      finishSubject();
      ...
      // Accept an unfamiliar learning-area label, including course codes such
      // as HC101, whether OCR puts grades on the same line or the next line.
      const isHeader = /learning\s*areas?|\bterms?\s*\d|\bquarters?\s*\d|final\s*grade|\bremarks?\b/i.test(label);
      if (label && /[a-z]/i.test(label) && !excludedLabels.test(label) && !isHeader) {
