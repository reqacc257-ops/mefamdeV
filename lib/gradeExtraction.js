...
Rules:
- Set gradingPeriodCount to 3 when the card has Terms/Periods 1-3 plus a final grade, or 4 when it has Quarters 1-4 plus a final grade.
...
function normalizeExtractionResult(raw) {
  const explicitPeriodCount = Number(raw?.gradingPeriodCount ?? raw?.periodCount);
  const normalizedExplicit = [1, 2, 3, 4].includes(explicitPeriodCount) ? explicitPeriodCount : null;
  const result = {
    gradingPeriodCount: normalizedExplicit,
  ...
  if (Array.isArray(raw?.subjects)) {
    result.subjects = raw.subjects.map(subject => {
      if (typeof subject === 'string') {
        ...
        return { name, q1: null, q2: null, q3: null, q4: null, final: null };
      }
  ...
  if (!result.gradingPeriodCount) {
    const maxKnown = Math.max(0, ...result.subjects.map(subject => [subject.q1, subject.q2, subject.q3, subject.q4].filter(value => value !== null).length));
    result.gradingPeriodCount = maxKnown >= 4 ? 4 : maxKnown >= 3 ? 3 : maxKnown >= 2 ? 2 : maxKnown >= 1 ? 1 : 3;
  }
...
function parseOcrSpaceText(text) {
  ...
  const finishSubject = () => {
    if (!current) return;
    const values = current.numbers;
    const hasThreeTerms = values.length === 4;
    subjects.push({
      name: current.name,
      q1: values[0] ?? null,
      q2: values[1] ?? null,
      q3: values[2] ?? null,
      q4: hasThreeTerms ? null : (values[3] ?? null),
      final: hasThreeTerms ? (values[3] ?? null) : (values[4] ?? null),
    });
    current = null;
  };
  String(text || '').split(/\r?\n/).forEach(line => {
    ...
    if (match) {
      ...
      const looksLikeCourseCode = /^[A-Z]{2,}[ -]?\d{2,}$/i.test(label);
      if ((numbers.length >= 2 || looksLikeCourseCode) && /[a-z]/i.test(label) && !excluded) {
  ...
  return normalizeExtractionResult({
    gradingPeriodCount: subjects.some(subject => subject.q4 !== null) ? 4 : 3,
    schoolYear: null,
    gradeLevel: null,
    subjects,
...
module.exports = { extractReportCard, computeFlags, GRADE_MIN, GRADE_MAX, normalizeExtractionResult, parseNumber, parseOcrSpaceText };
