...
async function extractReportCard({ fileData, fileType }) {
  ...
  try {
    ...
    data = await requestGemini(apiKey, mimeType, imageData, { maxOutputTokens: 800, temperature: 0 });
  }
  ...
  try {
    ...
    const retryData = await requestGemini(apiKey, mimeType, imageData, { maxOutputTokens: 800, temperature: 0 });
