...
async function extractReportCard({ fileData, fileType }) {
  if (String(process.env.GRADE_EXTRACTION_PROVIDER || '').toLowerCase() === 'ocr-space') {
    ...
    const ocrResult = await extractWithOcrSpace(ocrKey, mimeFromFileType(fileType), base64Only(fileData));
    if (!process.env.GEMINI_API_KEY) return ocrResult;
    // OCR.space can successfully process an image but return no usable table
    // rows. Fall through to Gemini when it is configured as a second reader.
  }
  ...
  const imageData = base64Only(fileData);
  ...
  let data;
  try {
    data = await requestGemini(apiKey, mimeType, imageData, structuredConfig);
  } catch (error) {
    if (error.code === 'GEMINI_RATE_LIMITED') throw error;
    data = await requestGemini(apiKey, mimeType, imageData, { maxOutputTokens: 2400, temperature: 0 });
  }
  ...
  try {
    ...
    if (Number(extracted?.confidence) >= 0.95) return extracted;

    const recheckPrompt = `${EXTRACTION_PROMPT}

RECHECK THIS IMAGE CAREFULLY. The first read was below 95% confidence. Inspect every visible grade cell again, preserve genuinely blank or unreadable cells as null, and do not increase confidence unless the image supports it. Return the complete JSON object again.`;
    const recheckData = await requestGemini(apiKey, mimeType, imageData, structuredConfig, recheckPrompt);
    const recheckText = recheckData.candidates?.[0]?.content?.parts?.find(part => part.text)?.text;
    if (!recheckText) return extracted;
    const rechecked = extractJsonFromText(recheckText);
    return Number(rechecked?.confidence) > Number(extracted?.confidence) ? rechecked : extracted;
  } catch (error) {
    const retryData = await requestGemini(apiKey, mimeType, imageData, { maxOutputTokens: 2400, temperature: 0 });
    const retryText = retryData.candidates?.[0]?.content?.parts?.find(part => part.text)?.text;
    if (!retryText) throw error;
    return extractJsonFromText(retryText);
  }
}
...
function computeFlags(extracted) {
  ...
  if (confidence < 0.95) {
    flags.push(`Overall extraction confidence is below 95% (${Math.round(confidence * 100)}%) — review carefully against the photo.`);
  }
