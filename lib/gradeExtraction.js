 ...
 *
 * Sends a (quality-checked) report card photo to Claude's vision API and
 ...
 *
 * Requires ANTHROPIC_API_KEY to be set in the environment (Railway → your
 * service → Variables). Get a key at https://console.anthropic.com/
 */
...
Rules:
- Only include top-level learning areas as rows in "subjects" (e.g. Filipino, English, Mathematics, Science, Araling Panlipunan, TLE, MAPEH, EsP). Skip MAPEH's sub-component rows (Music, Arts, PE, Health) — those are not separately graded subjects on the report card summary.
...
async function extractReportCard({ fileData, fileType }) {
      ...
      messages: [
        {
          ...
          content: [
            {
              type: 'image',
              source: { type: 'base64', media_type: mimeFromFileType(fileType), data: base64Only(fileData) },
            },
  ...
  if (!textBlock) {
    throw new Error('Extraction response contained no text content');
  }
...
module.exports = { extractReportCard, computeFlags, GRADE_MIN, GRADE_MAX };
