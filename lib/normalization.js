...
function getGradeRangeCategory(rawGrade, min, max) {
  const lowerBound = Number.isFinite(Number(min)) ? Number(min) : DEFAULT_GRADE_MIN;
  const upperBound = Number.isFinite(Number(max)) ? Number(max) : DEFAULT_GRADE_MAX;
  const verification = validateGrade(rawGrade, lowerBound, upperBound);

  if (!verification.valid && verification.reason?.startsWith('unparseable')) {
    return { category: 'unparseable', acceptable: false, normalizedValue: null, reason: verification.reason };
  }

  if (!verification.valid && verification.reason?.startsWith('grade out of range')) {
    return { category: 'out-of-range', acceptable: false, normalizedValue: verification.normalizedValue ?? null, reason: verification.reason };
  }

  return { category: 'acceptable', acceptable: true, normalizedValue: verification.normalizedValue ?? null, reason: verification.reason };
}

function isGradeAcceptable(rawGrade, min, max) {
  return getGradeRangeCategory(rawGrade, min, max).acceptable;
}
