...
async function normalizeLegacyAuditActors() {
  if (db.isPostgres) {
    try {
      await db.prepare(`
        UPDATE audit_logs
        SET user_name = 'Director', actor_role = 'director'
        WHERE lower(COALESCE(user_name, '')) = 'system'
           OR lower(COALESCE(actor_role, '')) = 'system'
      `).run();
    } catch (_) {}
    return;
  }
  ...
  let changed = false;
  logs.forEach(entry => {
    if (String(entry?.user || entry?.actorName || '').trim().toLowerCase() !== 'system'
      && String(entry?.actorRole || '').trim().toLowerCase() !== 'system') return;
    entry.user = 'Director';
    entry.actorName = 'Director';
    entry.actorRole = 'director';
    changed = true;
  });
  if (changed && typeof db.save === 'function') db.save();
}

async function listAuditLogs() {
  await purgePrintAuditLogs();
  await normalizeLegacyAuditActors();
  if (db.isPostgres) {
    try {
...
module.exports = { appendAuditLog, listAuditLogs, purgePrintAuditLogs, normalizeLegacyAuditActors, getAuditUser, getAuditLogsMemory };
